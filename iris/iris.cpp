...
int main(){
    ...
    for(int l = 1; l <= 3; l++){
        ...
        for(float lam = 0.0001; lam <= 1000; lam *= 10){
            ...
            int correct = 0;
            for(int i = 0; i < 75; i++){
                if(Ypred.at(i, 0) > Ypred.at(i, 1) && Ypred.at(i, 0) > Ypred.at(i, 2)){
                    if(Ytest.at(i, 0) == 1)
                        correct++;
                }else if(Ypred.at(i, 1) > Ypred.at(i, 0) && Ypred.at(i, 1) > Ypred.at(i, 2)){
                    if(Ytest.at(i, 1) == 1)
                        correct++;
                }else{
                    if(Ytest.at(i, 2) == 1)
            ...
            std::cout << "hidden layer: " << l << " lam: " << lam << std::endl;
        }
    }
